...
describe('Game class test',()=>{
  ...
  function gameGenerator(firstDice:number, secondDice:number): Game{
    mockedDiceRoller.roll.mockReturnValueOnce(firstDice).mockReturnValueOnce(secondDice);
 ...
 test('istnace should contain dices value between 1 and 6', ()=>{
  ...
  expect(player1.calcSuccesRate()).toEqual(50.00)
  const player2 = new Player("name", "password", [wonGame, wonGame, wonGame, lostGame])
  expect(player2.calcSuccesRate()).toEqual(75.00)
}
